...
def tles_to_satrec(tles: list[list[str]], gravity: GravityConstants | None = None) -> SatRec:
    ...
    Parameters
    ----------
    tles : list of list of str
        TLE lines with shape ``(n_sat, 2)``.  Each row contains
    ...
    -------
    SatRec
        A single :class:`SatRec` whose fields are stacked arrays with a
        leading dimension of ``n_sat``, ready for use with ``jax.vmap``.
    """
...
def satrec_from_elements(
    ...
    *,
    ...
    bstar : float, optional
        SGP4 drag term B*, km⁻¹.  Default 0.
    ndot : float, optional
        First derivative of mean motion, rad/min².  Default 0.
    nddot : float, optional
        Second derivative of mean motion, rad/min³.  Default 0.
    epoch_jd : float, optional
        Epoch as a full Julian date (e.g. 2451545.0 = J2000.0).
        Internally split into whole + fractional parts to preserve
        float64 precision.  Default J2000.0.
    gravity : GravityConstants, optional
        Gravity model constants.  Default :data:`WGS72`.
...
def propagate_jd(satrec: SatRec, jd: jax.typing.ArrayLike, fr: jax.typing.ArrayLike) -> tuple[jax.Array, jax.Array, jax.Array]:
    ...
    ----------
    satrec : SatRec
        Initialized SatRec from :func:`tle_to_satrec`.
    jd : array-like
        Julian date (UTC), integer/whole part (scalar).
    fr : array-like
        Julian date (UTC), fractional part (scalar).
...
def propagate_jd_leo(satrec: SatRec, jd: jax.typing.ArrayLike, fr: jax.typing.ArrayLike) -> tuple[jax.Array, jax.Array, jax.Array]:
    ...
    ----------
    satrec : SatRec
        Initialized SatRec from :func:`tle_to_satrec`.  Near-earth only.
    jd : array-like
        Julian date (UTC), integer/whole part (scalar).
    fr : array-like
        Julian date (UTC), fractional part (scalar).
...
def gcrf_positions_multi_leo(satrec: SatRec, times_jd: jax.typing.ArrayLike) -> tuple[jax.Array, jax.Array]:
    ...
    ----------
    satrec : SatRec
        Batched SatRec from :func:`tles_to_satrec`, all near-earth.
    times_jd : array-like, shape (n_times,)
        1-D array of UTC Julian dates.
...
def gcrf_positions_multi_sdp4_nr(satrec: SatRec, times_jd: jax.typing.ArrayLike) -> tuple[jax.Array, jax.Array]:
    ...
    ----------
    satrec : SatRec
        Batched SatRec from :func:`tles_to_satrec`, all deep-space ``irez=0``.
    times_jd : array-like, shape (n_times,)
        1-D array of UTC Julian dates.
